...
function ensureAgentsPanel(){
  ...
  <div class="modal-actions"><button class="primary-button" id="saveAgentBtn" type="button">Guardar agente</button></div>
...
function saveAgent(){
  const a={id:id(),name:document.getElementById('agentName').value.trim(),agency:document.getElementById('agentAgency').value.trim(),ami:document.getElementById('agentAmi').value.trim(),phone:document.getElementById('agentPhone').value.trim(),email:document.getElementById('agentEmail').value.trim(),defaultCommission:num(document.getElementById('agentDefaultCommission').value),notes:''};
  if(!a.name&&!a.agency){alert('Indique pelo menos o nome do agente ou a agência.');return}
  state.data.agents=state.data.agents||[];state.data.agents.push(a);save();renderAgents();populateAgentSelect();
  ['agentName','agentAgency','agentAmi','agentPhone','agentEmail','agentDefaultCommission'].forEach(x=>{const e=document.getElementById(x);if(e)e.value=''});
}
function renderAgents(){
  const box=document.getElementById('agentsList');if(!box)return;
  const list=state.data.agents||[];
  box.innerHTML=list.length?list.map(a=>`<div class="event-item"><strong>${esc(a.name||'Agente sem nome')}</strong><p class="muted">${esc(a.agency||'')} ${a.ami?'· AMI '+esc(a.ami):''} ${a.defaultCommission?'· Comissão padrão '+a.defaultCommission+'%':''}</p><p class="muted small">${esc(a.phone||'')} ${a.email?'· '+esc(a.email):''}</p><button class="ghost-button danger" type="button" data-delete-agent="${a.id}">Remover</button></div>`).join(''):'<div class="empty-state">Ainda não existem agentes cadastrados.</div>';
  box.querySelectorAll('[data-delete-agent]').forEach(btn=>btn.onclick=()=>{if(confirm('Remover este agente?')){state.data.agents=(state.data.agents||[]).filter(a=>a.id!==btn.dataset.deleteAgent);save();renderAgents();populateAgentSelect();}})
}
